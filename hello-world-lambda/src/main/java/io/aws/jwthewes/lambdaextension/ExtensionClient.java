...
//            "                \"SHUTDOWN\"," +
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ExtensionClient.class);
    ...
    public static String registerExtension() {
        ...
        try {
            ...
            response.headers().map().forEach((k, v) -> LOGGER.info("response header: " + k + " = " + String.join(",", v)));
            ...
            LOGGER.error("Error while registering extension: " + e.getMessage(), e);
        }
        LOGGER.info("returning null from registerExtension");
        return null;
    }
